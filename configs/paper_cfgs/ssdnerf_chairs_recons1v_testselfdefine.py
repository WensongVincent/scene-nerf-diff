...
data = dict(
    samples_per_gpu=4, # change this for batch size
    workers_per_gpu=4,
    train=dict(
        type=dataset_type,
    ...
    val_uncond=dict(
        type=dataset_type,
        data_prefix='data/shapenet/diy_chair_test',
        load_imgs=False,
        num_test_imgs=251,
        scene_id_as_name=True,
        cache_path='data/shapenet/chair_test_cache.pkl'),
    val_cond=dict(
        type=dataset_type,
        data_prefix='data/shapenet/diy_chair_test',
...
evaluation = [
    dict(
        ...
        feed_batch_size=2, # change this for feed_batch_size
        viz_step=32,
        metrics=dict(
            type='FID',
            num_images=4 * 250, # modify this if change dataset
...
custom_hooks = [
    dict(
        ...
        momentum_cfg=dict(
            ema_kimg=4, ema_rampup=0.05, batch_size=16, eps=1e-8), #batch_size=16
        priority='VERY_HIGH'),
    dict(
        type='SaveCacheHook',
